...
# Create your own attack
# ======================================================================
#you can create your own attack, choosing what kind nodes and links to add and remove from your attack
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if obj.get("type") == "node":
                ...
                end_obj = obj.get("end", {}) or {}

                start_props = start_obj.get("properties", {}) or {}
                end_props = end_obj.get("properties", {}) or {}

                start_name = start_props.get("name")
                end_name = end_props.get("name")

                start_id = str(start_obj.get("id", start_name))
                end_id = str(end_obj.get("id", end_name))
...
class ADAttackGenerator:
    def __init__(self, graph_json_path, graph_name="graph_0_probs.json"):
        self.G = load_graph_0(graph_json_path)
        self.graph_name = graph_name

    def node_id(self, n):
        return str(self.G.nodes[n].get("node_id", n))
    ...
    def path_node_names(self, path):
        return [self.node_name(n) for n in path]

    def path_node_ids(self, path):
        return [self.node_id(n) for n in path]
    ...
    def build_export_records(self, attacks, attack_name):
        ...
        for i, path in enumerate(attacks, 1):
            ...
            records.append({
                ...
                "relationships": self.path_relations(path),
                "length": len(path),

                "graph": self.graph_name,

                "path_name": self.path_node_names(path),
                "path_type": self.path_node_types(path)
            })
...
def launch_attack_generator_ui(
    ...
    default_attack_name="created_attack",
    export_dir="./attack_datasets",
    graph_name="graph_0.json"
):
    generator = ADAttackGenerator(graph_json_path, graph_name=graph_name)
    ...
    def on_generate_click(_):
        with output:
            ...
            print(f"Specific target       : {generator.node_name(target_node) if target_node else 'None'}")
            ...
            for i, path in enumerate(attacks, start=1):
                ...
                print(f"IDs       : {generator.path_node_ids(path)}")
